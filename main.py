...
from file_validator import FileValidator as validator
                    ...
                    try:
                        ...
                        # Validate PDF name:
                            ...
                            pdf_path = (
                                values["pdf_path"]
                                + "/"
                                + validator.validate_name(values["pdf_name"])
                            )
                ...
                if event_merge == "MERGE":
                    try:
                        ...
                        for i, path in enumerate(
                            ...
                            if len(path) >= 1:
                                if not validator.validate_pdf_file(path):
                        ...
                        # Validate PDF name:
                        elif validator.validate_name(values["merged_pdf_name"]) is None:
                            sg.popup_error("Please enter a name for a merged PDF file!")
                            window_merge["merged_pdf_name"].update("")
                        # Validate PDF output path:
                        elif not validator.validate_output_pdfpath(values["merged_pdf_path"]):
                            ...
                            merged_pdf_path = (
                                ...
                                + "/"
                                + validator.validate_name(values["merged_pdf_name"])
                            )
